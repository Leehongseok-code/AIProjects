...
hypothesisbe=torch.sigmoid(x_train[len(x_train)-1].matmul(W) + b)#마지막날을 원래모델에서는 어떻게 예측했는가!
hypothesis = torch.sigmoid(x_test.matmul(W) + b)#다음날을 모델에서 어떻게 예측했는가!
differ=hypothesis-hypothesisbe

print("train:",y_train[len(y_train)-1])
print("test:",x_test)
print("ppre:",hypothesisbe*mx+mn)
print("pre:",hypothesis*mx+mn)
'''
...
c.asia=c.asia+round(float(prediction[0]))
#print(prediction)
